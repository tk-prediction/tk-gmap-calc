...
class view_predict:
    def __init__(self, pred_res , val_min , val_max, intval , rep_dely , config ):
    ...
    #pngとして出力用
    def output_countour(self):
        folder = 'graphs' + os.sep
        ...
        for i in range( self.timesteps ):                 
            ...
            CR = plt.contour(  x , 
                               y , 
                               np.flipud( self.pred_res[i]) ,                                
                               np.arange(self.val_min , self.val_max , 0.05 ) ,
                               colors = 'black' , 
                               linewidth = 0.01 ) 
            CR.clabel(fmt='%1.1f', fontsize=10)
            '''
